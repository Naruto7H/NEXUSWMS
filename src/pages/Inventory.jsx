...
const StatusBadge = ({ status }) => {
  const styles = {
    Optimal: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400',
    'Low Stock': 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400',
    'Out of Stock': 'bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-400',
    Reorder: 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-400',
  };
...
export default function Inventory() {
  ...
  const handleExport = async () => {
    ...
    if(res.success) alert("Mock: Export initiated successfully.");
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium shadow-sm hover:bg-slate-50 transition-colors">
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto min-h-[400px]">
          {loading ? (
             <div className="p-8 flex justify-center text-slate-400">Loading master data...</div>
          ) : (
